...
import projekt.backEnd.exceptions.PlayerExceptionName;
import projekt.backEnd.repositories.PlayerRepository;
import projekt.backEnd.exceptions.PlayerExceptionID;
...
public class PlayerController {
    ...
    Player firstPlayer(@PathVariable Long id){
        return playerRepo.findById(id).orElseThrow(() -> new PlayerExceptionID(id));
    }
    ...
    Player findByName(@PathVariable String name){
        return playerRepo.findByName(name).orElseThrow(() -> new PlayerExceptionName(name));
    }
